...
uint32_t BreadCoinChecksum(uint8_t* p, uint32_t nBytes)
{
    if (!p || nBytes == 0)
        return 0;
...
{
    uint32_t checksum = BreadCoinChecksum(&data[0], data.size());
...
bool VerifyChecksum(const data_chunk& data)
{
    ...
    return BreadCoinChecksum((uint8_t*)&data[0], data.size()-4) == checksum;
};
