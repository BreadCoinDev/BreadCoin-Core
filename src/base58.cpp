...
namespace {
    class CBreadCoinAddressVisitor : public boost::static_visitor<bool> {
    private:
        CBreadCoinAddress *addr;
    public:
        CBreadCoinAddressVisitor(CBreadCoinAddress *addrIn) : addr(addrIn) { }
...
bool CBreadCoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CBreadCoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CBreadCoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CBreadCoinAddressVisitor(this), dest);
}

bool CBreadCoinAddress::IsValid() const {
...
CTxDestination CBreadCoinAddress::Get() const {
    if (!IsValid())
...
bool CBreadCoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CBreadCoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CBreadCoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CBreadCoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CBreadCoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CBreadCoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CBreadCoinSecret::SetString(const std::string& strSecret) {
