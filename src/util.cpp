...
int nAnonymizeBreadCoinAmount = 1000;
...
#else
    const char* pszModule = "BreadCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BREAD
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BREAD
    // Mac: ~/Library/Application Support/BREAD
    // Unix: ~/.BREAD
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BREAD";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BreadCoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "BreadCoin.conf";
               FILE* ConfFile = fopen(ConfPath.string().c_str(), "w");
               ...
               fprintf(ConfFile, "port=20375\n");
               fprintf(ConfFile, "rpcport=20425\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=172.105.121.51:20375\n");
               fprintf(ConfFile, "addnode=173.230.156.35:20375\n");
               fclose(ConfFile);
    }
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BreadCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BreadCoin will not work properly.");
