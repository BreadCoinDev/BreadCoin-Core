...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (BreadCoin users should be well used to small numbers with many decimal places by now :)
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (GetTime() - pnode->nTimeConnected > IDLE_TIMEOUT)
            {
                ...
                // Ping timeout - TODO : BreadCoiniew function
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "BreadCoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BreadCoin is probably already running."), addrBind.ToString());
        else
