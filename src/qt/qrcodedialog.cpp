...
{
    QString ret = QString("BreadCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BREAD unit input in lnReqAmount, we generate the URI with BREAD as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BreadCoinUnits::format(BreadCoinUnits::BREAD, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
