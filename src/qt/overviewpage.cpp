...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(BreadCoinUnits::BREAD)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseDarkTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = BreadCoinUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(BreadCoinUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(BreadCoinUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(BreadCoinUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(BreadCoinUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelTotal->setText(BreadCoinUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    //ui->labelWatchAvailable->setText(BreadCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    //ui->labelWatchStake->setText(BreadCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    //ui->labelWatchPending->setText(BreadCoinUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    //ui->labelWatchImmature->setText(BreadCoinUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    //ui->labelWatchTotal->setText(BreadCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    //ui->labelBREADtotalimg->setVisible(showWatchOnly);    // show watch-only balance separator line
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("BREAD")
    updateDisplayUnit();
}
...
{
    BreadCoinGUI::gotoHistoryPage_static();
}
