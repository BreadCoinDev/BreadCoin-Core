...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BreadCoinUnits::BREAD);
    ...
    if (!settings.contains("nAnonymizeBreadCoinAmount"))
        settings.setValue("nAnonymizeBreadCoinAmount", 1000);
    nAnonymizeBreadCoinAmount = settings.value("nAnonymizeBreadCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBreadCoinAmount"))
        SoftSetArg("-anonymizeBreadCoinamount", settings.value("nAnonymizeBreadCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBreadCoinAmount:
            return QVariant(nAnonymizeBreadCoinAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeBreadCoinAmount:
            nAnonymizeBreadCoinAmount = value.toInt();
            settings.setValue("nAnonymizeBreadCoinAmount", nAnonymizeBreadCoinAmount);
            emit AnonymizeBreadCoinAmountChanged(nAnonymizeBreadCoinAmount);
            break;
        default:
            break;
        }
