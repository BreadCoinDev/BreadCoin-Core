...
{
    widget->setMaxLength(BreadCoinAddressValidator::MaxAddressLength);
    widget->setValidator(new BreadCoinAddressValidator(parent));
...
bool parseBreadCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("BreadCoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BreadCoinUnits::parse(BreadCoinUnits::BREAD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBreadCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to BreadCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rev://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "BreadCoin:");
    }
    QUrl uriInstance(uri);
    return parseBreadCoinURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BreadCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BreadCoin.lnk
...
{
    return GetAutostartDir() / "BreadCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a BreadCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BreadCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BreadCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add BreadCoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BreadCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BreadCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BreadCoin-Qt"));
