...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "BreadCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "BreadCoin is downloading blocks...");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 128/4; i++)
            ((unsigned int*)pdata)[i] = ByteBreadCoinerse(((unsigned int*)pdata)[i]);
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BreadCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BreadCoin is downloading blocks...");
